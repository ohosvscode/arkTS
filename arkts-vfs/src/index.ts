...
declare let localStorage: LocalStorageLike | undefined
declare let fetch: FetchLike | undefined
...
export function createVirtualTypeScriptEnvironment(
  ...
  return {
    ...
    createFile: (fileName, content) => {
      // The `ets.createSourceFile` has the sixth parameter `options` (the compiler options), which is not supported by the TypeScript compiler,
      // so we need to use `@ts-expect-error` to ignore the type error.
      // @ts-expect-error
      updateFile(ts.createSourceFile(fileName, content, mergedCompilerOpts.target!, false, fileName.endsWith('.ets') ? 8 as any : undefined, compilerOptions))
    },
 ...
 *
 * @param compilerOptions The compiler settings target baseline
 * @param ts A copy of the TypeScript module
 */
export function knownLibFilesForCompilerOptions(compilerOptions: CompilerOptions, ts: TS): string[] {
  ...
  const getMax = (array: number[]): number | undefined =>
 ...
 */
export function createDefaultMapFromNodeModules(_compilerOptions: CompilerOptions, _ts?: typeof import('typescript'), tsLibDirectory?: string): Map<string, string> {
  const path = requirePath()
  const fs = requireFS()

  const getLib = (name: string): string => {
    // @ts-ignore
    const lib = tsLibDirectory || path.dirname(require.resolve('typescript'))
    return fs.readFileSync(path.join(lib, name), 'utf8')
  }

  const isDtsFile = (file: string): boolean => /\.d\.(?:[^.]+\.)?[cm]?ts$/i.test(file)
 ...
 */
export function addAllFilesFromFolder(map: Map<string, string>, workingDir: string): void {
  const path = requirePath()
  const fs = requireFS()
...
export function addFilesForTypesIntoFolder(map: Map<string, string>): void {
 ...
 */
export function createDefaultMapFromCDN(options: CompilerOptions, version: string, cache: boolean, ts: TS, lzstring?: LZString, fetcher?: FetchLike, storer?: LocalStorageLike): Promise<Map<string, string>> {
  ...
  function zip(str: string): string {
  ...
  function uncached(): Promise<void> {
    return (
...
function defaultCompilerOptions(ts: typeof import('typescript')): CompilerOptions {
  return {
...
const libize = (path: string): string => path.replace('/', '/lib.').toLowerCase()
 ...
 */
export function createVirtualCompilerHost(sys: System, compilerOptions: CompilerOptions, ts: TS): VirtualCompilerHost {
  const sourceFiles = new Map<string, SourceFile>()
  const save = (sourceFile: SourceFile): SourceFile => {
    sourceFiles.set(sourceFile.fileName, sourceFile)
    return sourceFile
  }

  const vHost: VirtualCompilerHost = {
    compilerHost: {
      ...sys,
        ...
        return (
          ...
          || save(
            ts.createSourceFile(
              ...
              false,
              fileName.endsWith('.ets') ? 8 as any : undefined,
              // The `ets.createSourceFile` has the sixth parameter `options` (the compiler options), which is not supported by the TypeScript compiler,
              // so we need to use `@ts-expect-error` to ignore the type error.
              // @ts-expect-error
              compilerOptions,
...
export function createVirtualLanguageServiceHost(
  ...
  const lsHost: VirtualLanguageServiceHost = {
    languageServiceHost,
    updateFile: (sourceFile) => {
      projectVersion++
...
function requirePath(): typeof import('node:path') {
  return require(String.fromCharCode(112, 97, 116, 104)) as typeof import('node:path')
}

function requireFS(): typeof import('node:fs') {
  return require(String.fromCharCode(102, 115)) as typeof import('node:fs')
}
