...
async function handleUseTemplate() {
  ...
  if (!fileUrl) return
  await connection.downloadAndExtractTemplate?.(fileUrl)
}
</script>

<template>
  <div :class="viewMode === 'grid' ? 'flex flex-col' : 'flex flex-row'" gap-16px @click="$router.push(`/project/template-detail?productId=${data.productId}`)">
